...
use bmlt-cli::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'bmlt-cli::Configuration' ) {
        $config = $_[0];
    } else {
        $config = bmlt-cli::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("bmlt-cli::Object::$class")->new;
        if (ref $data eq "HASH") {
